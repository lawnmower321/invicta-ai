...
import { User, Bell, Map, Key, Save, Check, Globe, AlertCircle } from "lucide-react";
...
export default function SettingsPage() {
  ...
  // notifications
  const [notifs, setNotifs] = useState({ newLead: true, followupDue: true, dealMoved: false, weeklyDigest: true });
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (p.partner_name) setPartnerName(p.partner_name);
      if (p.phone) setPhone(p.phone);
      if (p.company) setCompany(p.company);
      if (p.target_markets) setTargetMarkets(p.target_markets);
      if (p.min_price) setMinPrice(String(p.min_price));
      if (p.max_price) setMaxPrice(String(p.max_price));
      if (p.prop_types) setPropTypes(p.prop_types);
      if (p.mao_factor) setMaoFactor(String(p.mao_factor));
    });
  }, []);
  ...
  async function save() {
    ...
    await supabase.from("profiles").update({
      ...
      prop_types: propTypes.trim() || null,
      mao_factor: maoFactor ? Number(maoFactor) : 70,
    }).eq("id", userId);
    setSaved(true);
          ...
          {TABS.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button key={id} onClick={() => setTab(id)}
                ...
                style={{ background: active ? "var(--invicta-green)15" : "transparent", color: active ? "var(--invicta-green)" : "var(--muted-foreground)" }}>
                <Icon size={15} />{label}
              </button>
            );
          })}
          ...
          {tab === "profile" && (
            <div className="rounded-2xl border p-6 flex flex-col gap-5" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Email"><Input value={email} onChange={() => {}} readOnly /></Field>
              ...
              <div className="pt-4 border-t" style={{ borderColor: "var(--border)" }}>
                <p className="text-xs font-bold tracking-widest uppercase mb-3" style={{ color: "var(--muted-foreground)" }}>Change Password</p>
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <Field label="New Password"><Input value={newPassword} onChange={setNewPassword} type="password" placeholder="••••••••" /></Field>
                  <Field label="Confirm Password"><Input value={confirmPassword} onChange={setConfirmPassword} type="password" placeholder="••••••••" /></Field>
                </div>
                {passwordError && (
                  <div className="flex items-center gap-2 text-xs mb-3" style={{ color: "var(--invicta-red)" }}>
                    <AlertCircle size={12} />{passwordError}
                  </div>
                )}
                <button onClick={changePassword}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-sm transition-all"
                  style={{ background: passwordSaved ? "var(--invicta-green)20" : "var(--surface-3)", color: passwordSaved ? "var(--invicta-green)" : "var(--muted-foreground)" }}>
            ...
            <div className="rounded-2xl border p-6 flex flex-col gap-5" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
              ...
              <Field label="Target Markets (comma-separated)">
                <textarea value={targetMarkets} onChange={e => setTargetMarkets(e.target.value)}
                  rows={3} placeholder="Westchester NY, Bronx NY, Yonkers NY"
          ...
          {tab === "integrations" && (
            <div className="rounded-2xl border p-6 flex flex-col gap-5" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
              ...
              <div className="rounded-xl p-3 flex items-center gap-2"
                ...
                <p className="text-xs" style={{ color: "var(--invicta-blue)" }}>API keys are encrypted and never exposed in your browser.</p>
              </div>
              ...
              <p className="text-xs -mt-3" style={{ color: "var(--muted-foreground)" }}>Unlocks real-time MLS data, owner skip-tracing & advanced comps.</p>
              <Field label="BatchLeads API Key">
                <Input value={batchLeadsKey} onChange={setBatchLeadsKey} type="password" placeholder="batchleads-..." />
              </Field>
              <p className="text-xs -mt-3" style={{ color: "var(--muted-foreground)" }}>Enables bulk lead lists and direct mail integration.</p>
              <div className="pt-4 border-t" style={{ borderColor: "var(--border)" }}>
                <p className="text-xs font-bold tracking-widest uppercase mb-3" style={{ color: "var(--muted-foreground)" }}>Analytics</p>
